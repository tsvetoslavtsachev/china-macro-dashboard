...
- **Централен полярностен каталог** `catalog/polarity.py` — единен източник; модулите четат `polarity_for()`, не per-серия `invert` (затваря S7 CN-1 тихия fallback).
- **`z_score` = полярностно-ориентиран** (знак=здраве); суровият (над/под медиана) е в новото поле **`z_raw`**. Percentile носи прозореца си в **`percentile_window`** ("10г" / "пълна история").
- Инфлационната леща носи `comparability_note` — **несравнима между региони по дизайн** (CN линейна дефлационна леща vs US/EU U-форма; разликата ОСТАВА — тя е дизайн).
